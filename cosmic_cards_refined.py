...
HD_FLAVOR_MAPPING = {
    ...
    "Generator": "Respond and Satisfy",
    "Manifesting Generator": "Respond, Visualize, then Initiate",
    "Projector": "Wait for the Invitation",
    "Reflector": "Wait a Lunar Cycle",
}

# Term explanations data dictionary
TERM_EXPLANATIONS = {
    "Sun": {
        "title": "Sun Sign",
    ...
    "Rising": {
        "title": "Rising Sign",
        "body": "The mask you wear and the first impression you make. It's the lens through which the world sees you and you see the world.",
        "matters": "It dictates your spontaneous outward demeanor and the physical vitality filtering your chart."
    },
    "Birth Phase": {
        "title": "Birth Phase",
        "body": "The lunar phase at the exact moment you were born. It reveals your soul's innate rhythm — whether you're here to initiate, build, refine, or release.",
        "matters": "It aligns your natural energetic cadence with cosmic timing and developmental cycles."
    },
    "Full Moons Lived": {
        "title": "Full Moons Lived",
        "body": "The number of complete lunar cycles you've witnessed since birth. Each one is a chapter — a cycle of culmination and release you've lived through.",
    ...
    "HD Type": {
        "title": "HD Type",
        "body": "Your Human Design energy type — Manifestor, Generator, Manifesting Generator, Projector, or Reflector. It defines how you're designed to interact with the world and make decisions.",
        "matters": "It removes resistance by showing you how your aura naturally functions and exchanges energy."
    },
    "HD Flavor": {
        "title": "HD Flavor",
        "body": "Your Strategy and Authority in Human Design — the specific way your type is meant to navigate life. It's your personal decision-making compass.",
        "matters": "It provides the precise mechanical instructions for making correct choices without burnout."
    }
}

# Sun sign personality & purpose lines (NEVER starting with "You are.")
...
def build_card(user_hash: str) -> dict | None:
    ...
    try:
        ...
        hd_flavor = HD_FLAVOR_MAPPING.get(hd_type, "Respond and Satisfy")
...
def show_term_modal(term_key: str):
    """Helper to display term explanation in a popup or expander."""
    info = TERM_EXPLANATIONS.get(term_key)
    if not info:
        return
    with st.expander(f"📖 Explanation: {info['title']}", expanded=True):
...
def render_collectible_card(card: dict):
    """Immersive collectible card HTML with clickable sections and personality description."""
    n = card["natal"]
    ...
    hd_type = card.get("hd_type", "—")
    hd_flavor = card.get("hd_flavor", "—")
    ...
    ">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.55rem;">
        ...
        <div style="background:rgba(255,255,255,0.04);border-radius:10px;padding:0.55rem 0.7rem;border:1px solid rgba(255,255,255,0.07);">
          ...
          <div style="font-size:0.95rem;font-weight:600;color:#d2a8ff;">{hd_type} · {hd_flavor}</div>
        </div>
      </div>
    ...
    # Interactive Clickable Term Explanations
    st.markdown("##### 🔍 Tap a term below to explore its cosmic meaning:")
    cols_terms = st.columns(4)
    with cols_terms[0]:
        if st.button("Sun", use_container_width=True, key="btn_expl_sun"):
            st.session_state.active_term = "Sun"
        if st.button("Birth Phase", use_container_width=True, key="btn_expl_phase"):
            st.session_state.active_term = "Birth Phase"
    with cols_terms[1]:
        if st.button("Moon", use_container_width=True, key="btn_expl_moon"):
            st.session_state.active_term = "Moon"
        if st.button("Full Moons", use_container_width=True, key="btn_expl_moons"):
            st.session_state.active_term = "Full Moons Lived"
    with cols_terms[2]:
        if st.button("Rising", use_container_width=True, key="btn_expl_rising"):
            st.session_state.active_term = "Rising"
        if st.button("Dominant", use_container_width=True, key="btn_expl_dominant"):
            st.session_state.active_term = "Dominant"
    with cols_terms[3]:
        if st.button("HD Type", use_container_width=True, key="btn_expl_hdt"):
            st.session_state.active_term = "HD Type"
        if st.button("HD Flavor", use_container_width=True, key="btn_expl_hdf"):
            st.session_state.active_term = "HD Flavor"

    active_t = st.session_state.get("active_term")
    if active_t:
        show_term_modal(active_t)

    # User Personality & Purpose Description based on Sun Sign
    sun_sign = n["sun_sign"]
    desc = SUN_SIGN_DESCRIPTIONS.get(sun_sign, "Radiant and purposeful, you embody unique cosmic gifts.")
    st.markdown("---")
    st.markdown("##### ✨ Cosmic Archetype & Purpose")
    st.info(f"**{sun_sign} Sun Archetype:** {desc}")
