...
def render_card_back(card: dict):
    ...
    def get_hd_interpretation(hd_type: str) -> str:
        ...
        return hd_descriptions.get(hd_type, f"As a {hd_type}, you have a unique energetic design. Your purpose is to live in alignment with your design.")
    ...
    def get_element_breakdown(sun: str, moon: str, rising: str | None) -> dict:
        ...
        signs = [sun, moon]
        if rising: signs.append(rising)
        counts = {}
        for s in signs:
            if s in elements:
                e = elements[s]
                counts[e] = counts.get(e, 0) + 1
        if not counts: return {"dominant": "Balanced", "description": "Your chart has a harmonious balance of elements, allowing you to adapt and flow through life with grace."}
        most = max(counts, key=counts.get)
        elem_descriptions = {
            "Fire": "Passionate, energetic, and driven. Inspire others with enthusiasm and courage. Path: action and inspiration.",
            "Earth": "Grounded, practical, and reliable. Build stability and nurture growth. Path: structure, patience, and tangible results.",
            "Air": "Intellectual, communicative, and curious. Thrive on ideas, connection, and mental exploration. Path: understanding and connection.",
            "Water": "Emotional, intuitive, and deeply feeling. Navigate life through empathy and emotional depth. Path: compassion and healing.",
        }
        return {"dominant": most, "description": elem_descriptions.get(most, "Your elements are balanced, offering you a wide range of strengths and perspectives.")}
    ...
    def get_phase_interpretation(phase: str) -> str:
        phase_meanings = {
            "New Moon": "You are a seed, full of potential. Here to begin new cycles, plant intentions, and nurture what is unborn. Life themes: initiation and fresh starts.",
            "Waxing Crescent": "You are a sprout, reaching toward the light. Here to gather momentum, build confidence, and take the first steps toward your intentions. Path: growth and early action.",
            "First Quarter": "You are a sapling, stretching in all directions. Here to make decisions, take leaps, and develop your strength. Growth comes through action and choice.",
            "Waxing Gibbous": "You are a young tree, establishing your roots. Here to refine, improve, and perfect your path. Journey: refinement and dedication.",
            "Full Moon": "You are a full tree, in full bloom. Here to illuminate, inspire, and lead by example. Path: culmination, expression, and celebration.",
            "Waning Gibbous": "You are a harvest, sharing your fruits. Here to share wisdom, give thanks, and guide others. Life: giving back and teaching.",
            "Last Quarter": "You are a season of letting go. Here to release what no longer serves, break patterns, and prepare for renewal. Lessons: surrender and forgiveness.",
            "Waning Crescent": "You are a seed, returning to the earth. Here to rest, dream, and renew. Path: quiet reflection and deep dreaming.",
        }
    ...
    <div style="background: linear-gradient(160deg, #0a0e17 0%, #12101f 40%, #0d1f3c 100%); border: 2px solid {sun_c}; border-radius: 20px; padding: 1.25rem 1.35rem 1.4rem; margin: 0.8rem 0 1.2rem; box-shadow: 0 0 32px {sun_c}44, inset 0 0 40px rgba(0,0,0,0.35); position: relative; overflow: hidden;">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.6rem;margin-bottom:0.6rem;">
        <div style="background:rgba(255,255,255,0.03);border-radius:12px;padding:0.7rem;border:1px solid rgba(255,255,255,0.06);"><div style="font-size:0.55rem;color:#8b949e;font-weight:700;letter-spacing:1px;">🔥 ELEMENT — {elem['dominant']}</div><div style="font-size:0.8rem;color:#e6edf3;line-height:1.4;margin-top:0.2rem;">{elem['description']}</div></div>
        <div style="background:rgba(255,255,255,0.03);border-radius:12px;padding:0.7rem;border:1px solid rgba(255,255,255,0.06);"><div style="font-size:0.55rem;color:#8b949e;font-weight:700;letter-spacing:1px;">🌙 BIRTH PHASE — {n['phase_emoji']} {phase}</div><div style="font-size:0.8rem;color:#e6edf3;line-height:1.4;margin-top:0.2rem;">{phase_meaning}</div></div>
      </div>
      <div style="background:rgba(255,255,255,0.03);border-radius:12px;padding:0.7rem;margin-bottom:0.6rem;border:1px solid rgba(255,255,255,0.06);">
...
def resolve_trade(trade_id: int, user_hash: str, accept: bool):
    ...
    c.execute("UPDATE card_trades SET status=?, resolved_at=CURRENT_TIMESTAMP WHERE id=?", (status, trade_id))
    conn.commit()
    conn.close()
    return True
...
def render_profile_form(user_hash: str, key_prefix: str = "cards"):
    ...
    with c1:
        ...
        except Exception: t_val = dtime(12, 0)
